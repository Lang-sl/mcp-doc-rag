...
要在文档搜索之上增加源代码搜索，切换到网关适配器（推荐）或直接网关模式：

1. 复制并编辑网关配置模板：

```bash
cp src/rag/gateway.example.yaml gateway.yaml
# 编辑 gateway.yaml：设置 doc_rag.config_path 和 codegraph.cwd（可选）
```
...
# 可选的 CodeGraph 集成。移除本区块则网关仅提供文档搜索。
codegraph:
  command: "npx"
...
2. 更新 `.mcp.json` 使用网关适配器（推荐）或直接网关：
